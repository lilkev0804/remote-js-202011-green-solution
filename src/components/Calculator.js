        ...
        const request = await axios.get("https://koumoul.com/s/data-fair/api/v1/datasets/base-carbone(r)/values_agg?field=Nom_base_fran%C3%A7ais&format=json&agg_size=20&q=camion&q_mode=complete&size=20&select=&highlight=")
            ...
            this.state.camions.map(camion => (
                camion.results.filter(result => (
                    result["Identifiant_de_l'élément"].includes("21066") || result["Identifiant_de_l'élément"].includes("21074") || result["Identifiant_de_l'élément"].includes("21052") || result["Identifiant_de_l'élément"].includes("28023") ? vehicules.push(result.Total_poste_non_décomposé) : ""
                    )
                )
            )
            )
                ...
                <div className= "calculatorBloc">
                    <div>
                        ...
                        <label for="vehicume" className="calculator-titleBox">Choix du véhicule : </label>
                            <select name="vehicule" id="vehiculeSelect" onClick={this.handleClick} className="vehiculeList" >
                                <option className="vehiculeList" value={this.state.emissionFactors[9]} >Vehicule utilitaire 3,5T</option> 
